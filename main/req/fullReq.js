...
import * as mimic from '../mid/backendProcessor/MimicAuthInsert.js';
...
uploadRemote.connect(async function(err, res) {
    ...
    tokenRemote.connect(async function(err, res) {
        ...
        while(true) {
            ...
            let recvAddr = userAccount.normalAccount[2].address;
            ...
            copyrightInfoArr.forEach(async copyrightInfo => {
                copyrightInfo.workId = workId;
                copyrightInfo = copyrightInfo;
                let tokenId = sha256(workId + copyrightInfo.copyrightType).toString();
                console.log('copyright:', workId + '-' + copyrightInfo.copyrightType);
                await erc721.buildPubTokenTx(tokenRemote, fakeBaiduAuthorizeAddr, fakeBaiduAuthorizeSecr, tokenSeq++, recvAddr, tokenName.copyright, tokenId, copyrightInfo, false);
            })
