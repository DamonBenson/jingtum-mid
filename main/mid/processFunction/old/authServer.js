...
import {chains, authMemo, rightTokenName, ipfsConf, debugMode} from '../../../utils/info.js';
...
function handleAuth(request, response) {
    ...
    request.on('data', async function(chunk) {
        ...
        for(let i = 0; i < 17; i++) {
            ...
            tokenIssuePromises.push(erc721.buildIssueTokenTx(r, seq++, rightTokenName, tokenId, tokenMemos, true)); //银关填写通证memos
            tokenAuthPromises.push(erc721.buildAuthTokenTx(r, seq++, addr, rightTokenName, tokenId, true)); //银关将通证转让给用户（目前井通sdk发行通证需要这两步，导致确权开销较大）
        }
