...
r.connect(async function(err, result) {
    ...
    r.on('ledger_closed', async function(msg) {
        ...
        const uncheckCount = uncheckInfoArr.length;
        const loopCounter = uncheckCount - 1;

        // 获取存证交易信息
        // [0]=n
        let txInfoPromises = new Array(uncheckCount);
        for(let i = loopCounter; i >= 0; i--) {
            let uncheckInfo = uncheckInfoArr[i];
            txInfoPromises[i] = requestInfo.requestTx(r, uncheckInfo.hash, false);
        }
        let txArr = await Promise.all(txInfoPromises);

        // 根据存证交易中的哈希标识获取作品信息
        // [0]=0
        let txMemosArr = new Array(uncheckCount);
        for(let i = loopCounter; i >= 0; i--) {
            let tx = txArr[i];
            txMemosArr[i] = JSON.parse(localUtils.ascii2str(tx.Memos[0].Memo.MemoData).slice(2));
        }
        // [0]=n
        let workInfoGetPromises = new Array(uncheckCount);
        for(let i = loopCounter; i >= 0; i--) {
            let txMemos = txMemosArr[i];
            workInfoGetPromises[i] = ipfsUtils.get(ipfs, txMemos.workInfoHash);
        }
        let workInfoJsonArr = await Promise.all(workInfoGetPromises);

        // 整合作品存证信息，向版权局发送确权请求，并获取确权ID
        // [0]=0
        let authReqPromises = new Array(uncheckCount);
        for(let i = loopCounter; i >= 0; i--) {
            let tx = txArr[i];
            let txMemos = txMemosArr[i];
            let workInfo = JSON.parse(workInfoJsonArr[i]);
            delete txMemos.workInfoHash;
            let authInfo = Object.assign(workInfo, txMemos);
            authInfo.addr = tx.Destination;
            authInfo.uploadtime = tx.date + 946684800; // 加一项上传时间
            if(debugMode) {
            ...
            authReqPromises[i] = fetch.postData('http://127.0.0.1:9000/authReq', authInfo);
        }
        let authResArr = await Promise.all(authReqPromises);

        // 更新mysql中作品信息中的auth_id
        let authPromises = new Array(uncheckCount);
        for(let i = loopCounter; i >= 0; i--) {
            let uncheckInfo = uncheckInfoArr[i];
            let authRes = authResArr[i];
            let authId = authRes.body._readableState.buffer.head.data.toString();
            let workId = uncheckInfo.work_id;
            authPromises[i] = mysqlUtils.update(c, 'work_info', "auth_id = '" + authId + "'", "work_id = '" + workId + "'");
        }
